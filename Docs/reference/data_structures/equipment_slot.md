...
Crafting, as well as many interactions involving equipping and stashing, are currently *hard-coded* to expect up to two "hand" slots:
a `RIGHT HAND` and a `LEFT HAND`.
There is currently no other way to define hands for use in interactions involving hands.
Players can be missing their `LEFT HAND`, but this will forbid them from engaging in hand-crafting.
`RIGHT HAND`s are expected to come before `LEFT HAND`s on the Inventory Items sheet.
Moderators may encounter erroneous behavior if they fail to conform to this expectation.
