...
export const config = {
    ...
    details: `Lists all occupants currently in the given room. If an occupant is in the process of moving, their move `
        + `queue will be included, along with the time remaining until they reach the next room in their queue. Note `
        + `that the displayed time remaining will not be adjusted according to the \`HEATED_SLOWDOWN_RATE\` setting. `
        + `If a player in the game has the \`heated\` status effect, movement times for all players will be displayed `
        + `as shorter than they actually are. Occupants with the \`hidden\` behavior attribute `
        + `will also be listed alongside their hiding spots.`,
    usableBy: "Moderator",
    aliases: ["occupants", "o"],
    requiresGame: true
};
...
export async function execute(game, message, command, args, moderator) {
    ...
    if (occupantsList.length === 0) occupantsMessage = `There is no one in ${room.displayName}.`;
    else occupantsMessage += `__All occupants in ${room.displayName}:__\n` + occupantsList.join(" ");
